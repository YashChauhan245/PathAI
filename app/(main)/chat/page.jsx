...
import { Bot, Sparkles, Send, FileText, ArrowRight, AlertCircle } from "lucide-react";
...
export default function PathAIChatPage() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const result = await sendChatMessage(promptText);
      if (result.success) {
        setMessages((prev) => [
          ...prev,
          {
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {error && (
          <div className="rounded-lg border border-red-900 bg-red-950/30 p-3 flex items-start gap-2 text-sm text-red-200">
                ...
                {isTyping && (
                  <div className="max-w-[220px] rounded-2xl border border-[#252525] bg-[#0f0f0f] px-4 py-3">
                    <div className="typing-dots flex items-center gap-1">
                      ...
                      <span className="h-2 w-2 rounded-full bg-zinc-400 animate-pulse" style={{ animationDelay: "0.2s" }} />
                      <span className="h-2 w-2 rounded-full bg-zinc-400 animate-pulse" style={{ animationDelay: "0.4s" }} />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 rounded-2xl border border-[#242424] bg-[#0b0b0b] p-2 transition focus-within:neon-border">
                  <Input
                    ...
                    className="border-0 bg-transparent shadow-none focus-visible:ring-0 disabled:opacity-50"
                  />
                  <Button 
                    onClick={handleSend} 
                    size="icon" 
              ...
              <CardContent className="space-y-2">
                {quickPrompts.map((prompt) => (
                  <button
                    ...
                    disabled={isTyping}
                    className="w-full rounded-xl border border-[#252525] bg-[#101010] px-3 py-2 text-left text-sm text-zinc-200 transition hover:bg-[#151515] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {prompt}
                  </button>
                ))}
